...
async function updateAttendance(request_id, staff_id) {
  try {
    const [rows, fieldDefs] = await pool.query(
      `UPDATE tsh.${tableName} SET attendance = 1 WHERE request_id = ? AND staff_id = ?`,
      [request_id, staff_id]
    );
...
async function getStaffsByRequestId(request_id) {
  try {
    const [rows] = await pool.query(
      `
            SELECT Training.staff_id, staff_name FROM tsh.${tableName}
            LEFT JOIN Staff
...
async function createMany(request_id, staff_ids) {
  try {
    const query = pool.format(
      "INSERT INTO tsh.Training (request_id, staff_id)  VALUES ?",
