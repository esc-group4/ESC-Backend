...
async function findById(id) {
  try {
    const [rows, fieldDefs] = await pool.query(
      `SELECT * FROM tsh.${tableName} WHERE staff_id = ?`,
      [id]
    );
    return rows.map((row) => new Staff(row));
  } catch (error) {
...
async function getByFirebaseUid(firebaseUid) {
  try {
    const [rows, fieldDefs] = await pool.query(
      `SELECT staff_id, staff_name, staff_email, position as role FROM tsh.${tableName}
        LEFT JOIN tsh.Designation
        ON tsh.Staff.designation_id = tsh.Designation.designation_id
        WHERE firebase_uid = ? 
        LIMIT 1`,
      [firebaseUid]
    );
